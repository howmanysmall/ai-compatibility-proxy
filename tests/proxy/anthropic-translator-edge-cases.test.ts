...
describe("anthropic Translator Edge Cases", () => {
	it("translateOpenAiToAnthropic rejects weird unsupported request fields", () => {
	...
	it("translateOpenAiToAnthropic rejects invalid choice counts and missing user content", () => {
		expect.hasAssertions();
		expectProxyError(
	...
	it("translateOpenAiToAnthropic rejects tool/function message shapes", () => {
		expect.hasAssertions();
		const cases = [
	...
	it("translateOpenAiToAnthropic validates token edge cases", () => {
		expect.hasAssertions();
		expectProxyError(
			() =>
				translateOpenAiToAnthropic(
	...
	it("translateOpenAiToAnthropic supports text content arrays and string stop", () => {
		expect.assertions(7);
		...
		expect(translated.stop_sequences, "Expected string array stop to pass through.").toStrictEqual(["END", "STOP"]);
		...
		expect(stringStopRequest.stop_sequences, "Expected string stop to normalize to Anthropic array.").toStrictEqual(
			["END"],
		);
	});

	it("translateAnthropicToOpenAi ignores non-text blocks and maps fallback response fields", () => {
		expect.assertions(4);
		...
		expect(response.choices[0]?.message.content, "Expected only text blocks to be concatenated.").toBe(
			"hello world",
		);
	...
	it("anthropic finish and usage mappers cover nullish and refusal edges", () => {
		expect.assertions(7);
