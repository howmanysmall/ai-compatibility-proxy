...
function hasKeyAttribute(node: ESTree.JSXElement): boolean {
...
function isHigherOrderComponent({ callee }: ESTree.CallExpression): boolean {
...
function getCallbackUsageFromReference(
	...
	if (callExpression === undefined || isHigherOrderComponent(callExpression)) return EMPTY_CALLBACK_USAGE;
...
function getJSXAttributeName({ name }: ESTree.JSXAttribute): string | undefined {
