...
function parseOptions(rawOptions: unknown): EffectFunctionOptions {
	if (!isRecord(rawOptions)) return { environment: "standard", hooks: DEFAULT_HOOKS };

	const environment: Environment = isEnvironment(rawOptions.environment) ? rawOptions.environment : "standard";
