...
function hasWordBoundary(previous: string, current: string, next: string | undefined): boolean {
	if (isLowerCaseLetter(previous) && isUpperCaseLetter(current)) return true;
	if (isUpperCaseLetter(previous) && isUpperCaseLetter(current) && next !== undefined && isLowerCaseLetter(next)) {
		return true;
	}
	if (isAlphabetic(previous) && isDigit(current)) return true;
	if (isDigit(previous) && isAlphabetic(current)) return true;
	return false;
}
...
function splitIdentifierIntoWords(identifier: string): ReadonlyArray<string> {
	...
	for (let index = 1; index < identifier.length; index += 1) {
		...
		if (!hasWordBoundary(previous, current, next)) continue;

		words.push(identifier.slice(start, index));
		start = index;
	}

	words.push(identifier.slice(start));
	return words;
}
...
function normalizeAllowList(options: unknown): Map<string, boolean> {
	...
	const mergedAllowList = extendDefaultAllowList
		? mergeObjects(DEFAULT_ALLOW_LIST, configuredAllowList)
		: (configuredAllowList ?? {});
...
function normalizeReplacements(options: unknown): Map<string, Map<string, boolean>> {
	...
	for (const discouragedName of replacementKeys) {
		...
		const mergedForName = configuredOverride === false ? {} : mergeObjects(baseReplacements, configuredOverride);
